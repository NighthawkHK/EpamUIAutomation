...
public class Task1 extends BaseTest {
    ...
    public void verifyThatNameOfHeadlineArticleEqualsToExpected() {
        assertThat(new HomePage(getDriver()).goToNewsPage().getTitleOfHeadlineArticle())
    ...
    public void verifyThatSecondaryArticleTitlesOfHeadlineListAreEqualToExpected() {
        assertThat(new HomePage(getDriver()).goToNewsPage().getAllTitlesFromTimelineList().equals(EXPECTED_TITLES_OF_TIMELINE_LIST))
    ...
    public void verifyThatNameOfTheFirstArticleContainsValue() {
        String textOfCategoryLinkOfMainNews = new HomePage(getDriver()).goToNewsPage().getTextOfCategoryLinkOfMainNews();
        new NewsPage(getDriver()).executeSearchByKeyword(textOfCategoryLinkOfMainNews);
        assertThat(new SearchResultPage(getDriver()).getFirstArticleHeadline())
